...
sql_file = open(cfg.pvc_query, "r")
pvc_query = sql_file.read()
sql_file.close()
...
def main():
    vmi_df = pd.DataFrame(pd.read_sql_query(pvc_query, engine))
    vmi_df.columns = map(str.upper, vmi_df.columns)

    writer = pd.ExcelWriter(
        f"c:\\users\\u6zn\\desktop\\xlsx\\Weekly PVC Inventory Report {timestring}.xlsx"
    )

    vmi_df.to_excel(writer, "PVC REPORT", index=False)
    workbook = writer.book
    worksheet = writer.sheets["PVC REPORT"]
...
if __name__ == "__main__":
    ...
    mail.To = "David.Kole@oncor.com; Tea.Chavez@oncor.com"
    mail.Subject = f"PVC Report {timestring}"
    mail.Attachments.Add(f"c:\\users\\u6zn\\desktop\\xlsx\\Weekly PVC Inventory Report {timestring}.xlsx")
    mail.HtmlBody = (
