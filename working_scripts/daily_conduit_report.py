...
sql_file = open(cfg.conduit_query, "r")
conduit_query = sql_file.read()
sql_file.close()
...
def main():
    conduit_df = pd.DataFrame(pd.read_sql_query(conduit_query, engine))
...
if __name__ == "__main__":
    ...
    mail.Attachments.Add(f"c:\\users\\u6zn\\desktop\\xlsx\\Daily 886 Conduit Inventory Report {timestring}.xlsx")
    mail.HtmlBody = (
    ...
    xl_workbook = open_workbook(f"c:\\users\\u6zn\\desktop\\xlsx\\Daily 886 Conduit Inventory Report {timestring}.xlsx", on_demand=True)
